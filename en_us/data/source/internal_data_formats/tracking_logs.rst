...
The student and instructor events are grouped into categories in this section. For
...
``event`` fields contain. For more infomration about the common ``context`` or
...
bumper video. This option is only available for the bumper video file that a
course team can include for a course, and allows the user to opt out of
viewing the bumper file in the future.

**Event Source**: Browser

**History**: Added 8 Jun 2015.

``event`` **Member Fields**: 

The ``do_not_show_again_video`` events include the following ``event`` member
fields. These fields serve the same purpose for events of this type as for the
:ref:`play_video` events.

* ``code``
* ``currentTime``: The point in the video file at which the user opted out of
  the bumper video, in seconds.
* ``id``
* ``is_bumper``: True.
...
This option is only available for the bumper video file that a course team can
include for a course, and allows the user to play the selected video
immediately.
...
The ``skip_video`` events include the following ``event`` member fields. These
fields serve the same purpose for events of this type as for the
:ref:`play_video` events.

* ``code``
* ``currentTime``: The point in the video file at which the bumper video was
  skipped, in seconds.
* ``id``
* ``is_bumper``: True.
...
For more informaiion about the common fields, see :ref:`common`.
